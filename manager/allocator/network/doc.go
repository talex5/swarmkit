...
// local state for many reasons. All of the libnetwork libraries that the
// network allocator depends on also have their own local state. If the local
// state of the allocator is not completely consistent with the distributed
// state, then errors such as double allocations or use-after-frees may result.
//
// In order to correctly allocate, the allocator provides a Restore method,
// to which all allocated and unallocated objects can be passed. The Restore
// method runs through the state of all objects (and calls Restore methods on
// the subcomponents) to fully initialize the local state of the allocator. If
// the Restore method returns no errors, then the local state has been fully,
...
// ipam, and port packages. These components ere each concerned with specific
